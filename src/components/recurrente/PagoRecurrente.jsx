...
function PagoRecurrente({ egreso, onClose, onSave }) {
  const [formData, setFormData] = useState({
    fecha: new Date().toISOString().split('T')[0],
    sucursal: '',
    monto: egreso?.cuotaMes || 0,
    metodoPago: {
      tipo: 'efectivo',
      numeroVoucher: '',
      numeroCheque: '',
    ...
    if (formData.incluirFlujoCaja && formData.metodoPago.tipo === 'efectivo') {
      try {
        await registrarMovimientoEfectivo({
          ...
          detalle: `Pago egreso recurrente - ${egreso?.categoria}: ${egreso?.detalleCuenta}`,
          tipo: 'egreso',
          ...
          <form onSubmit={handleSubmit}>
            <div className="modal-body">
              <div className="alert alert-info">
                <strong>Categoría:</strong> {egreso?.categoria}<br />
                <strong>Detalle:</strong> {egreso?.detalleCuenta}<br />
                <strong>Cuota:</strong> ${egreso?.cuotaMes?.toLocaleString()}
              </div>

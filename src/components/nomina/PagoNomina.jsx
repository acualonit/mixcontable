...
function PagoNomina({ empleado, onClose, onSave }) {
  const [formData, setFormData] = useState({
    fecha: new Date().toISOString().split('T')[0],
    sucursal: '',
    monto: empleado?.sueldo || 0,
    metodoPago: {
      tipo: 'efectivo',
      numeroVoucher: '',
      numeroCheque: '',
    ...
    if (formData.incluirFlujoCaja && formData.metodoPago.tipo === 'efectivo') {
      try {
        await registrarMovimientoEfectivo({
          ...
          detalle: `Pago nómina - ${empleado?.nombreCompleto}`,
          tipo: 'egreso',
          categoria: 'Nómina',
          ...
          <form onSubmit={handleSubmit}>
            <div className="modal-body">
              <div className="alert alert-info">
                <strong>Empleado:</strong> {empleado?.nombreCompleto}<br />
                <strong>Documento:</strong> {empleado?.numeroDocumento}<br />
